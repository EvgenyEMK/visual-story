/**
 * Presentation card for the dashboard grid — shows thumbnail, inline rename, and action menu.
...
export function PresentationCard({
  ...
  const commitRename = () => {
    ...
    if (trimmed && trimmed !== presentation.name) {
      onRename(presentation.id, trimmed);
    } else {
    ...
    >
      ...
      <div className="p-3">
        {isRenaming ? (
          ...
          <h3 className="truncate text-sm font-medium">{presentation.name}</h3>
        )}

        <div className="mt-1 flex items-center justify-between text-xs text-muted-foreground">
          <span>{presentation.slideCount} slides</span>
          <span>{formatRelativeTime(presentation.updatedAt as unknown as string)}</span>
        </div>
      </div>
