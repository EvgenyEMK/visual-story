...
import { Rocket, Shield, Zap, BarChart3, Target, Eye, Cloud, Send, FileText } from 'lucide-react';
...
// ---------------------------------------------------------------------------
// Layout Skeletons tab
// ---------------------------------------------------------------------------

function LayoutSkeletonsSection({ showHeader }: { showHeader: boolean }) {
  return (
    <DemoGrid>
      {LAYOUT_TEMPLATES.map((layout) => (
        <div key={layout.id} className="space-y-2">
          <SlideLayoutSkeleton layout={layout} showHeader={showHeader} />
          <div className="px-1">
...
// ---------------------------------------------------------------------------
// Tab sections — real slide examples
// ---------------------------------------------------------------------------

function GridLayoutsSection() {
...
function NavigationLayoutsSection() {
  const { themeMode } = useDemoPage();
  return (
    <DemoGrid>
      {/* SidebarDetail */}
...
function CompositeLayoutsSection() {
  const { themeMode } = useDemoPage();
  return (
    <DemoGrid>
      {/* TitleSlide */}
...
export function SlideLayoutTemplatesClient() {
  const [showHeader, setShowHeader] = useState(true);

  return (
    <DemoPageShell
      title="Slide Layout Templates"
      description="Content layout skeletons and interactive slide-level compositions. Toggle the header checkbox to see how layouts adapt."
      extraControls={
        <label className="flex items-center gap-2 cursor-pointer select-none">
          ...
          />
          <span className="text-sm font-medium text-muted-foreground">Show Header</span>
        </label>
      }
      tabs={[
        { value: 'skeletons', label: 'Layout Skeletons', count: LAYOUT_TEMPLATES.length, content: <LayoutSkeletonsSection showHeader={showHeader} /> },
        { value: 'grids', label: 'Grid Layouts', count: 3, content: <GridLayoutsSection /> },
        { value: 'navigation', label: 'Navigation Layouts', count: 4, content: <NavigationLayoutsSection /> },
        { value: 'composite', label: 'Composite', count: 2, content: <CompositeLayoutsSection /> },
      ]}
    />
  );
}
